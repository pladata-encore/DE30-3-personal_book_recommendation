...
**2. 수집 방식**<br />
앞서 설명한 어려움으로 인해 크롤링이 아닌 "100자평", "마이리뷰" 2개 모두 알라딘 내부 API를 활용하였다. 각각 다른 API를 사용하여 수집하였고 API 호출 시 미리 수집된 a_isbn과 iteml_id를 파라미터로 사용하였다 마이리뷰의 경우 헤더 정보를 함께 전달해야했다. response는 html형식이었으며 파싱 작업을 통해 원하는 정보를 csv파일로 저장하였다.<br />
**3. 데이터 병합**
