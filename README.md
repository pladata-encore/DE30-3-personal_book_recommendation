...
- 동작방식: 입력한 키워드를 기반으로 수집된 리뷰 데이터를 분석하여 사용자에게 적합한 책을 추천하는 프로세스를 구현하였다. 키워드는 쉼표(,)로 구분하여 여러개 입력할 수 있다.<br />
![](https://velog.velcdn.com/images/devysy55/post/c8a12905-8910-4e68-ae47-ef2045be53d2/image.png)<br />

- 분석 방식  <br />
   -  키워드와 리뷰 간의 유사도 계산은 TF-IDF 벡터라이저를 사용하였으며 코사인 유사도를 계산하여 각 리뷰마다 유사도 점수를 추출한다.<br />
    - 유사도 점수를 기반으로 각 책별로 유사도의 합을 계산하고, 이를 내림차순으로 정렬하여 상위 3권의 책을 시각화한다.<br />
    - 추가적으로 유사도 높은 리뷰의 비율이 높은 책 상위 3권의 책도 시각화하였다.<br />
- 인사이트 도출<br />
    - '각 책별 유사도 합을 계산하는 방식'과 '유사도 높은 리뷰의 비율을 계산하는 방식' 2가지를 활용하였는데 테스트 결과 후자가 더 유의미한 추천을 제공하였다.<br />
- 아쉬운 점<br />
    - TF-IDF 벡터라이저와 코사인 유사도 계산에 대한 이해가 부족한 상태로 검색을 통해 코드를 작성하여 적절하지 활용, 개선하지 못했다. 좀 더 공부하고 연구하여 수정이 필요하다.
